<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Gerenciar Usuário do Aplicativo') }}: {{ $appUser->name }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 md:p-8 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('admin.app-users.update', $appUser) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $appUser->name)" required autofocus />
                            ...
                            <div>
                                ...
                                <x-text-input id="email" class="block mt-1 w-full" type="email" name="email" :value="old('email', $appUser->email)" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="cpf" class="block mt-1 w-full" type="text" name="cpf" :value="old('cpf', $appUser->cpf)" placeholder="000.000.000-00"/>
                            ...
                            <div>
                                ...
                                <x-text-input id="phone" class="block mt-1 w-full" type="text" name="phone" :value="old('phone', $appUser->phone)" placeholder="(00) 90000-0000"/>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <x-text-input id="company_name" class="block mt-1 w-full" type="text" name="company_name" :value="old('company_name', $appUser->company_name)" />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="client_id" name="client_id" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    ...
                                    @foreach ($clients as $client)
                                        <option value="{{ $client->id }}" {{ old('client_id', $appUser->client_id) == $client->id ? 'selected' : '' }}>
                                            {{ $client->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="service_point_id" name="service_point_id" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                        ...
                        <div class="mt-4">
                            ...
                            <select id="approval_status" name="approval_status" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required>
                                <option value="pending" {{ old('approval_status', $appUser->approval_status) == 'pending' ? 'selected' : '' }}>{{ __('Pendente') }}</option>
                                <option value="approved" {{ old('approval_status', $appUser->approval_status) == 'approved' ? 'selected' : '' }}>{{ __('Aprovado') }}</option>
                                <option value="rejected" {{ old('approval_status', $appUser->approval_status) == 'rejected' ? 'selected' : '' }}>{{ __('Rejeitado') }}</option>
                            </select>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const currentServicePointId = "{{ old('service_point_id', $appUser->service_point_id) }}";

        function fetchServicePoints(selectedClientId, currentSpId) {
                ...
                .then(data => {
                    ...
                    if (data.length > 0) {
                        data.forEach(sp => {
                            ...
                            if (currentSpId && parseInt(currentSpId) === sp.id) {
        ...
        if (clientIdSelect) {
            clientIdSelect.addEventListener('change', function () {
                fetchServicePoints(this.value, null); // Reset current SP when client changes
            });

            // Initial load if a client is already selected
            if (clientIdSelect.value) {
                fetchServicePoints(clientIdSelect.value, currentServicePointId);
            } else {
                servicePointIdSelect.disabled = true;
            }
        }
