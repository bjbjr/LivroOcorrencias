<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Editar Posto de Serviço') }}: {{ $servicePoint->name }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('admin.service-points.update', $servicePoint) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mt-4">
                            ...
                            <select id="contract_id" name="contract_id" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required>
                                ...
                                @foreach ($contracts as $contract)
                                    <option value="{{ $contract->id }}" {{ (old('contract_id', $servicePoint->contract_id) == $contract->id) ? 'selected' : '' }}>
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $servicePoint->name)" required autofocus />
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="internal_code" class="block mt-1 w-full" type="text" name="internal_code" :value="old('internal_code', $servicePoint->internal_code)" />
                        ...
                        <div class="grid md:grid-cols-2 gap-6 mt-4">
                            ...
                            <div>
                                ...
                                <x-text-input id="latitude" class="block mt-1 w-full" type="text" name="latitude" :value="old('latitude', $servicePoint->latitude)" placeholder="-23.550520" />
                                <x-input-error :messages="$errors->get('latitude')" class="mt-2" />
                            </div>
                            <!-- Longitude -->
                            <div>
                                <x-input-label for="longitude" :value="__('Longitude (Opcional)')" />
                                <x-text-input id="longitude" class="block mt-1 w-full" type="text" name="longitude" :value="old('longitude', $servicePoint->longitude)" placeholder="-46.633308" />
