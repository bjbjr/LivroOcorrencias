<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Editar Contrato') }}: {{ $contract->name }}
        </h2>
    </x-slot>
                    ...
                    <form action="{{ route('admin.contracts.update', $contract) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $contract->name)" required autofocus />
                            ...
                            <div>
                                ...
                                <x-text-input id="contract_code" class="block mt-1 w-full" type="text" name="contract_code" :value="old('contract_code', $contract->contract_code)" required />
                            ...
                            <div>
                                ...
                                <select id="status" name="status" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required>
                                    <option value="active" {{ old('status', $contract->status) == 'active' ? 'selected' : '' }}>{{ __('Ativo') }}</option>
                                    <option value="pending" {{ old('status', $contract->status) == 'pending' ? 'selected' : '' }}>{{ __('Pendente') }}</option>
                                    <option value="inactive" {{ old('status', $contract->status) == 'inactive' ? 'selected' : '' }}>{{ __('Inativo') }}</option>
                                    <option value="expired" {{ old('status', $contract->status) == 'expired' ? 'selected' : '' }}>{{ __('Expirado') }}</option>
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="start_date" class="block mt-1 w-full" type="date" name="start_date" :value="old('start_date', $contract->start_date ? $contract->start_date->format('Y-m-d') : '')" />
                            ...
                            <div>
                                ...
                                <x-text-input id="end_date" class="block mt-1 w-full" type="date" name="end_date" :value="old('end_date', $contract->end_date ? $contract->end_date->format('Y-m-d') : '')" />
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="details" name="details" rows="4" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">{{ old('details', $contract->details) }}</textarea>
